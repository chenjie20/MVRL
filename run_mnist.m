...
mnist_labels = mnist_labels + 1;

n = 1000;
total_num = length(mnist_labels);
...
for alpha_idx = 1 : alpha_num
    ...
    for dim_idx = 1 : length(dims)
        ...
        for wnd_idx = 1 : num_windows
            ...
            for nv_idx = 1 : nv                                 
                if wnd_idx == 1
                    original_data_views{nv_idx} = X{nv_idx}(:, start_idx : start_idx + n - 1);

                else
                    original_data_views{nv_idx} = [X{nv_idx}(:, start_idx : start_idx + n - 1), Xs{nv_idx}(:, :)];
                end
            end
            ...
            dlmwrite('mvrl_mnist_data_parameters.txt', [wnd_idx, alpha, dim, acc, nmi, purity, fmeasure, ratio, time_cost] , '-append', 'delimiter', '\t', 'newline', 'pc');   
        end
        ...
        dlmwrite('mvrl_mnist_avg_results_parameters.txt', [alpha, dim, average_acc, average_nmi, average_purity, average_fm, average_ratio, average_cost] , '-append', 'delimiter', '\t', 'newline', 'pc');
    end
end
save('mvrl_mnist_results_final_parameters.mat', 'final_clustering_accs', 'final_clustering_nmis', 'final_clustering_purities', 'final_clustering_fmeasures', 'final_clustering_ratios', 'final_clustering_costs');
